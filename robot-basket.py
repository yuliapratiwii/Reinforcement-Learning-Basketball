...
class QLearning:
    def __init__(self, alpha=0.1, gamma=0.9, epsilon=0.95, epsilon_decay=0.99, epsilon_min=0.01, render=False, logging=True):
        self.alpha = alpha 
        self.gamma = gamma 
...
class TDLambda:
    def __init__(self, alpha=0.01, gamma=0.8, lambd=0.9, epsilon=0.95, epsilon_decay=0.99, epsilon_min=0.01, render=False, logging=True):
        self.alpha = alpha 
        self.gamma = gamma 
        self.epsilon = epsilon 
        self.epsilon_min = epsilon_min 
        ...
        self.n_actions = len(self.env.action_dict)
        self.Q_table = np.zeros((self.n_states, self.n_actions))
        self.E_table = np.zeros((self.n_states, self.n_actions))  # Eligibility traces table
        self.logging = logging
        if self.logging:
            self.logger = Logger("/tmp/scalar/reward", "/tmp/scalar/avg-reward")
    ...
    def learn(self, n_episodes, n_steps):
        ...
        for episode in range(n_episodes):
            ...
            for step in range(n_steps):
                ...
                # SARSA update rule
                current_Q = self.Q_table[state_number, action]
                next_Q = self.Q_table[next_state_number, next_action]
                delta = reward + self.gamma * next_Q - current_Q

                # Update eligibility trace
                self.E_table[state_number, action] += 1
            ...
            print(f"Episode: {episode}, Total Reward: {sum_reward}")
            total_reward += sum_reward
            avg_reward = total_reward / (episode + 1)
            list_reward.append(sum_reward)
            list_avg_reward.append(avg_reward)
...
def main():
    ql = QLearning(alpha=0.1, gamma=0.9, epsilon=0.95, epsilon_decay=0.99, epsilon_min=0.1, render=True, logging=True)
    ql.learn(9100, 50)
